    ...
    <div class="bg-[#0d0d0d] min-h-screen w-full">
      <div class="max-w-7xl mx-auto px-4 py-8 mt-14 sm:py-12 lg:py-16">
        <ProfileHeaderSection
          ...
          :stats="{ posts: totalPost, likes: totalLikes, favorites: totalFavorites }"
        />
...
onMounted(async () => {
  try {
    ...
    userPhotos.value = photosResponse?.photos?.map(photo => ({
      ...photo,
    ...
    userVideos.value = videosResponse?.videos?.map(video => ({
      ...video,
    ...
    // Calculate stats
    const photoLikes = userPhotos.value.reduce((sum, photo) => sum + (photo.user_comments?.reduce((cSum, comment) =>
      cSum + (comment.user_reactions?.length || 0), 0) || 0), 0)
    const videoLikes = userVideos.value.reduce((sum, video) => sum + (video.user_comments?.reduce((cSum, comment) =>
      cSum + (comment.user_reactions?.length || 0), 0) || 0), 0)
