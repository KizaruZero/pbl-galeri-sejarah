<template>
    <div class="relative flex flex-col bg-black px-5">
        ...
        <section class="p-10 w-full bg-black">
            <div class="grid grid-cols-3 gap-14 mx-auto max-w-[1192px] max-md:grid-cols-2 max-sm:grid-cols-1">
                <ImageCard v-for="event in events" :key="event.slug" :imageUrl="event.imageUrl" :title="event.title"
                    :titleSize="event.titleSize" :description="event.description" :videoUrl="event.videoUrl"
                    @play-video="openVideoPlayer" />
            </div>
        </section>

        <!-- Video Player Modal -->
        <VideoPlayer :isOpen="isVideoPlayerOpen" :videoUrl="currentVideoUrl" @close="closeVideoPlayer" />
    </div>
</template>
