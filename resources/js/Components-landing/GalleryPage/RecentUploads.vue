<template>
    <section class="w-full bg-white dark:bg-black py-16 px-6 md:px-10">
        <div class="max-w-[1192px] mx-auto">
            ...
            <!-- Empty State -->
            <div v-else-if="!photos.length && !videos.length"
            ...
            <div v-else>
                ...
                <div v-if="photos.length > 0">
                    <h2 class="text-2xl font-serif text-black dark:text-white mb-6">
                        Foto Terbaru
                    </h2>
                    <div class="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-12">
                        <div v-for="item in photos.slice(0, 3)" :key="item.id"
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="w-8 h-8 rounded-full overflow-hidden bg-gray-700">
                                            <img :src="item.userAvatar" :alt="item.userName" class="w-full h-full object-cover" />
                                        </div>
                                        <!-- User Name -->
                                        <span class="text-white text-sm font-medium">{{ item.userName }}</span>
                                    </div>
                            ...
                            <div class="p-4 text-black dark:text-white">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 mt-2" v-if="item.description">
                                    {{ item.description }}
                                </p>
            ...
            photos.value = photoResponse.data
                .map((item) => ({
                    ...
                    userAvatar: item.user ?.photo_profile ?
                        `/storage/${item.user.photo_profile.replace(/^public\//, "")}` :
                        "/js/Assets/default-avatar.jpg",
                    viewsCount: item.total_views || 0,
                    likesCount: item.likes_count || item.content_reactions_count || 0,
