<template>
                ...
                <div v-if="user" class="relative group" ref="dropdownRef">
                    ...
                    >
                        ...
                        >
                            <Link
                                href="/profile-page"
                                class="block px-4 py-2 hover:bg-gray-900 dark:hover:bg-gray-100"
                                >Profile</Link
                            >
                            <Link
                                @click="goToDashboard"
    ...
    <transition name="slide">
        <div
            v-if="menuOpen"
            class="fixed inset-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm z-50 flex"
        >
            <div
                class="w-56 bg-white h-full dark:bg-black/80 p-6 flex flex-col gap-6"
            >
                <!-- Close Button -->
                <button
                    @click="toggleMenu"
                    class="text-black dark:text-white self-end text-2xl"
                >
                    ✕
                </button>
                ...
                <template v-if="user">
                    <template
                        v-if="
                            roles.some((role) =>
                                ['super_admin', 'author', 'direktur'].includes(
                                    role
                                )
                            )
                        "
...
const goToDashboard = () => {
    if (roles.value.includes("author")) {
...
const getMediaUrl = (url) => {
    ...
    return cleanPath ? `/storage/${cleanPath}` : defaultPhoto;
};
