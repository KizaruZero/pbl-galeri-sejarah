<template>
  <footer class="bg-white dark:bg-black text-black dark:text-white py-10 px-5 md:px-20">
    <div class="flex flex-col md:grid md:grid-cols-3 gap-10">
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <div class="flex flex-col leading-tight">
            <h2
              v-if="!isLoading && companyProfile?.cms_name"
              class="text-xl md:text-2xl uppercase font-semibold"
            >
      ...
      <div class="w-full flex justify-center">
        ...
        >
          ...
          <Link :href="user ? '/contact' : '/member'" class="hover:underline">{{
            user ? "Contact" : "Member"
          }}</Link>
    ...
    <!-- Copyright Section -->
    <div class="w-full border-t border-gray-300 dark:border-gray-700 mt-8 pt-4">
      <p class="text-center text-sm">
        © {{ new Date().getFullYear() }} {{ companyProfile?.cms_name || 'Loading...' }}. 
        Developed by Team Empat. All rights reserved.
      </p>
    </div>
  </footer>
</template>
