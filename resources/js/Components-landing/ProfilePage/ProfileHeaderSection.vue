                ...
                <div class="text-black dark:text-white text-center md:text-left px-2 sm:px-0">
                    ...
                    <div class="flex justify-end gap-4 mb-4 mt-2">
                        <div class="flex flex-col-2 gap-4">
                            ...
                            <div class="flex flex-col gap-2">
                                <button @click="openImportModalVideo"
                                    class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-200">
                                ...
                                <button @click="downloadTemplateVideo"
                                    ...
                                    Download Video Upload Template
                                </button>
                            </div>

                            <!-- Photo Buttons -->
                            <div class="flex flex-col gap-2">
                                <button @click="openImportModalPhoto"
                                    class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <button @click="downloadTemplatePhoto"
                                    class="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    Download Photo Upload Template
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="isImportModalOpenVideo"
                        ...
                        <div class="bg-gray-200 dark:bg-gray-800 rounded-lg p-6 w-96">
                            ...
                            <div v-if="importStatus" class="mb-4 p-3 rounded"
                                :class="importStatus.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'">
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                <button @click="submitImportVideo" :disabled="!selectedFile || isImporting"
                                    class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-500 disabled:bg-blue-300">
        ...
        <div v-if="isFavoritesModalOpen"
            ...
            <div class="bg-white rounded-lg p-6 w-96 max-h-[80vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="closeFavoritesModal" class="text-gray-500 hover:text-gray-700">
                ...
                <div v-if="favoritesList?.length > 0" class="space-y-3">
                    <div v-for="favorite in favoritesList" :key="favorite.id"
                        ...
                        <img :src="favorite.user_photo ? `/storage/${favorite.user_photo}` : defaultPhoto"
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Menyukai "{{ favorite.post_title }}"</p>
                        </div>
                    </div>
                </div>
...
<script setup>
    ...
    const downloadTemplateVideo = () => {
        const link = document.createElement('a');
        link.href = 'storage/excel-video/Template_Upload_Video.xlsx';
        link.download = 'Video_Upload_Template_Video.xlsx';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    // Download Template photo
    const downloadTemplatePhoto = () => {
        const link = document.createElement('a');
        link.href = 'storage/excel-photo/Template_Upload_Photo.xlsx';
        link.download = 'Photo_Upload_Template_Photo.xlsx';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const showFavoritesModal = async () => {
        try {
            const response = await axios.get(`/api/favorite/total/user/${props.userData.id}`, {
                headers: {
    ...
    const submitImportVideo = async () => {
        ...
        try {
            ...
            let errorMessage = error.response ?.data ?.message || 'Error mengimpor file';
