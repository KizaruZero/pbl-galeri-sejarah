    ...
    <div class="min-h-screen text-white">
        ...
        <div class="mt-6 sm:mt-8 md:mt-12 container mx-auto">
            <div v-if="activeTab === 'photo'"
                class="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 gap-1 sm:gap-2 md:gap-4">
                <div v-for="photo in props.photos" :key="'photo-' + photo.id"
                    class="rounded-md sm:rounded-lg shadow-md cursor-pointer relative group">
                    <!-- Edit button and status in top right corner -->
                    ...
                    'bg-blue-500 text-white': photo.status === 'pending',
                        ...
                        <!-- Edit button -->
                        <button class="bg-black bg-opacity-80 hover:bg-opacity-100 rounded-full p-2 shadow-md"
                    ...
                    <!-- Photo container - Added click handler -->
                    <div class="aspect-square overflow-hidden relative" @click="navigateToPhotoDetail(photo)">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                            <div class="text-white font-medium text-lg line-clamp-1">
                                {{ photo.title }}
                            </div>
                        </div>
                    </div>

                    <!-- Rejection note (appears below photo) -->
                    <div v-if="photo.status === 'rejected'" class="mt-1 text-center text-sm font-medium text-red-200">
                        Rejected: {{ photo.note }}
                    </div>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'video'"
                ...
                <div v-for="video in props.videos" :key="'video-' + video.id"
                    class="overflow-hidden relative rounded-md sm:rounded-lg shadow-md cursor-pointer">
                    ...
                    'bg-gray-500 text-white': video.status === 'pending',
                }">
                            {{ video.status }}
                        </span>
                        <!-- Edit button -->
                    ...
                    <div class="aspect-square overflow-hidden relative" @click="openVideo(video)">
                        ...
                        <div
                            class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 sm:p-3">
                            <div class="text-white font-medium text-sm sm:text-base line-clamp-1">
                                {{ video.title }}
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Rejection note -->
                    <div v-if="video.status === 'rejected'" class="mt-1 text-center text-sm font-medium text-red-200">
                        Rejected: {{ video.note }}
                    </div>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'favorit'" class="container mx-auto">
                ...
                <div v-if="
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 mx-auto mb-4" viewBox="0 0 24 24"
                        fill="currentColor">
                        <path fill-rule="evenodd"
